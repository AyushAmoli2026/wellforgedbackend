...
export const signup = async (req, res) => {
    ...
    try {
        ...
        const newUser = await pool.query('INSERT INTO users (first_name, last_name, email, mobile_number, password) VALUES ($1, $2, $3, $4, $5) RETURNING id, first_name, last_name, email, mobile_number, role', [first_name, last_name, email, mobile_number, password]);
...
export const getProfile = async (req, res) => {
    try {
        const userResult = await pool.query('SELECT id, first_name, last_name, email, mobile_number, role, is_verified, created_at FROM users WHERE id = $1', [req.user.id]);
        if (userResult.rows.length === 0) {
